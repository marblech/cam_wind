 ...
 * 协议文档 3.2 节说明：本协议中所有以0x0F，0xF0为帧头的报文，
 * 除伺服控制器外，数据位长度固定15字节，未使用部分默认为00。
 */
 ...
 * @brief 负载状态上报数据长度
 * 
 * 协议 3.4.1 节定义：数据位包含 41 字节（字节号 5~45，对应负载状态上报数据区）
 * 注意：协议表3-12中字节号从帧头开始计算，数据区从字节号5开始到字节号45结束，共41字节
 */
 ...
 * @brief 负载状态上报数据结构
 * 
 * 对应协议 3.4.1 节 "负载状态上报" 定义 (表3-11/3-12)。
 * 帧格式: [1F F1][地址][帧序号][数据41字节][校验][F1 1F]
 * 总长度: 49 字节
 * 
 * 协议文档中字节号从帧头开始计算，数据区从字节号5开始。
 * 以下为数据区各字段定义（方括号内为协议文档中的字节号）：
 * 
 * - [5]: 伺服工作模式 (0x00:停止; Bit3~bit0:伺服模式; Bit5~bit4:跟踪源; Bit7~Bit6:自动模式)
 * - [6-9]: 红外焦距 (float型)
 * - [10-13]: 白光/可见光焦距 (float型)
 * - [14-17]: 方位脱靶量 (float型)
 * - [18-21]: 俯仰脱靶量 (float型)
 * - [22-25]: 环控设备状态 (4字节，格式: 01**02**，**：00关闭/FF打开; 01:雨刷 02:加热)
 * - [26-29]: 伺服方位角 (float型)
 * - [30-33]: 伺服俯仰角 (float型)
 * - [34]: 自检状态 (Bit0:0分系统正常,1异常; Bit1:0白光相机正常,1异常; Bit2:0红外相机正常,1异常; Bit3:0跟踪处理器正常,1异常; Bit4:0雨刷正常,1异常; Bit5:0加热器正常,1异常; Bit6:0自检结束,1自检中; Bit7:备用)
 * - [35]: 可见光摄像机图像状态 (00:有图像, 01:设备1无图像)
 * - [36]: 可见光摄像机通信状态 (00:有通信, 01:设备1无通信)
 * - [37]: 红外摄像机图像状态 (00:有图像, 01:设备1无图像)
 * - [38]: 红外摄像机通信状态 (00:有通信, 01:设备1无通信)
 * - [39]: 伺服故障码 (0:正常, 其他:异常)
 * - [40]: 电子透雾状态 (00:无效; 01:有效)
 * - [41]: 雨刷状态 (0:工作中, 1:空闲)
 * - [42]: 加热状态 (0:加热中, 1:空闲)
 * - [43]: 俯仰伺服故障码 (0:正常, 其他:异常)
 * - [44-45]: 当前记忆跟踪时间 (uint16_t, 毫秒, 0000-1388即0-5000毫秒)
 */
struct LoadStatusReport {
    ...
    // 数据区字段 (协议字节号从5开始)
    uint8_t servo_work_mode = 0;             ///< [5] 伺服工作模式
    //   Bit3~bit0: 伺服模式 (1000:测试模式, 1001:伺服位置, 1010:伺服速度, 1011:跟踪模式)
    //   Bit5~bit4: 跟踪源 (01:可见, 10:红外, 11:自动)
    //   Bit7~Bit6: 模式 (10:自动模式默认, 01:半自主模式)
    float ir_focal_length = 0.0f;            ///< [6-9] 红外焦距 (float)
    float vis_focal_length = 0.0f;           ///< [10-13] 白光/可见光焦距 (float)
    float az_aberration = 0.0f;              ///< [14-17] 方位脱靶量 (float)
    float el_aberration = 0.0f;              ///< [18-21] 俯仰脱靶量 (float)
    
    // 环控设备状态 (4字节)
    uint8_t env_ctrl_byte1 = 0;              ///< [22] 环控设备1状态字节 (雨刷)
    uint8_t env_ctrl_byte2 = 0;              ///< [23] 环控设备2状态字节
    uint8_t env_ctrl_byte3 = 0;              ///< [24] 环控设备3状态字节 (加热)
    ...
    uint8_t ir_cam_image_status = 0;         ///< [37] 红外摄像机图像状态 (00=有图像, 01=无图像)
    uint8_t ir_cam_comm_status = 0;          ///< [38] 红外摄像机通信状态 (00=有通信, 01=无通信)
    uint8_t servo_fault_code = 0;            ///< [39] 伺服故障码 (0=正常, 其他=异常)
    uint8_t defog_status = 0;                ///< [40] 电子透雾状态 (00=无效, 01=有效)
    uint8_t wiper_status = 0;                ///< [41] 雨刷状态 (0=工作中, 1=空闲)
    uint8_t heater_status = 0;               ///< [42] 加热状态 (0=加热中, 1=空闲)
    uint8_t el_servo_fault_code = 0;         ///< [43] 俯仰伺服故障码 (0=正常, 其他=异常)
    uint16_t memory_track_time = 0;          ///< [44-45] 当前记忆跟踪时间 (毫秒, 0-5000)
...
struct Packet {
    ...
    uint8_t checksum = 0;                ///< 和校验 (从地址到数据的逐字节累加取低8位)
 ...
 * @note 函数名含 xor 仅为兼容旧代码，实际实现为和校验
 */
