...
public interface CamMonLibrary extends Library {
    ...
    /**
     * JNA 的浮点数引用类型（用于 get_ptz 输出参数）
     * 使用纯 JNA Memory 类来分配和管理 native 内存，不依赖 jna.platform 包
     */
    public static class FloatReference {
        private Memory memory;
        
        public FloatReference() {
            this.memory = new Memory(4);  // 4 bytes for float
            writeFloat(0f);
        }
        
        public FloatReference(float value) {
            this.memory = new Memory(4);
            writeFloat(value);
        }
        
        public void writeFloat(float value) {
            memory.write(0, new float[]{value}, 0, 1);
        }
     ...
     */
    int cam_controller_get_last(Pointer h, Pointer buf, int buflen);
    
    /**
     * 从最后一条状态包中获取解析后的 PTZ 值（指定 IP 来源）
     * 
     * C 签名: int cam_controller_get_ptz(CamController* h, const char* ip, float* out_az, float* out_el, 
     *                                     float* out_ir_focus, float* out_vis_focus)
     * 
     * @param h 控制器句柄
     * @param ip 来源 IP 地址（null 表示使用最后接收的）
     * @param outAz 输出口位角（需要预先创建长度为 1 的数组）
     * @param outEl 输出俯仰角（需要预先创建长度为 1 的数组）
     * @param outIrFocus 输出红外焦距（需要预先创建长度为 1 的数组）
     * @param outVisFocus 输出可见光焦距（需要预先创建长度为 1 的数组）
     * @return 若有有效值返回 1，否则返回 0
     */
    int cam_controller_get_ptz(Pointer h, String ip, 
