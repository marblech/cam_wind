...
public class DllTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 从 JAR 包中提取原生库到临时目录，并设置 jna.library.path
        // 必须在任何 JNA 接口首次访问之前调用
        NativeLoader.extractNativeLibraries();

        // 第一步：启动本地 UDP 回显服务器（模拟摄像头/舵机设备）
        System.out.println("[1/5] 启动 UDP 回显服务器（端口 " + ECHO_PORT + "）...");
        Thread echoServer = startEchoServer();
        Thread.sleep(500); // 等待服务器启动

        // 第二步：加载 DLL 并创建控制器
        System.out.println("[2/5] 加载 cammon.dll 并创建控制器...");
        boolean started = CamMonNative.startStatusListener(LISTEN_PORT);
        if (!started) {
        ...
        if (ptz != null) {
            ...
            System.out.println("    el (俯仰角):      " + String.format("%.2f", ptz.el) + "°");
            System.out.println("    irFocus (红外焦距): " + String.format("%.2f", ptz.irFocus) + "mm");
            System.out.println("    visFocus (可见光焦距): " + String.format("%.2f", ptz.visFocus) + "mm");
        } else {
            System.out.println("  无有效 PTZ 数据（设备可能未返回完整数据包）");
        }
        System.out.println();

        // 第六步：清理资源
        System.out.println("[6/6] 清理资源...");
