...
public class CamMonNative {
    ...
    public static boolean startStatusListener(int port) {
        return startStatusListener(port, "239.255.43.21");
    ...
    public static byte[] getLastStatus() {
        ...
        com.sun.jna.Memory mem = new com.sun.jna.Memory(2048);
        int n = CamMonLibrary.INSTANCE.cam_controller_get_last(g_controller, mem, 2048);
        ...
        System.arraycopy(mem.getByteArray(0, n), 0, result, 0, n);
        return result;
    }
    
    /**
     * 获取最后解析的 PTZ 数据（使用最后接收的数据源 IP）
     * 
     * 对应 C++ 接口：cam_controller_get_ptz(h, nullptr, ...)
     * 
     ...
     * 获取指定 IP 来源的 PTZ 数据
     * 
     * 对应 C++ 接口：cam_controller_get_ptz(h, ip, ...)
     * 
     * @param sourceIp 来源 IP 地址（null 表示使用最后接收的）
     * @return PTZStatus 对象（包含 az, el, irFocus, visFocus），无数据时返回 null
     */
    public static PTZStatus getPTZ(String sourceIp) {
        if (g_controller == null) {
            return null;
        }
        ...
        int ok = CamMonLibrary.INSTANCE.cam_controller_get_ptz(
            g_controller, sourceIp, az, el, irFocus, visFocus);
